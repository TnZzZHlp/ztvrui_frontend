...
function _cleanMac(m) {
  ...
  for (let i = 0; i < m.length && m2.length < 17; ++i) {
    ...
    if ('0123456789abcdef'.indexOf(c) >= 0) {
      m2 += c
      if (m2.length > 0 && m2.length !== 17 && (m2.length & 1) === 0) m2 += ':'
    }
  }
  return m2
}
...
function _renderMatches(mtree, rules, macros, caps, tags, params) {
  ...
  for (let k = 0; k < mtree.length; ++k) {
    const match = typeof mtree[k][0] === 'string' ? mtree[k][0].toLowerCase() : ''
    if (match.length === 0 || match === 'and') {
      ...
      const args = []
      const argCount = MATCH_ARG_COUNTS[match]
      if (!argCount) return [mtree[k][1], mtree[k][2], 'Unrecognized match type "' + match + '".']
      ...
      switch (match) {
        ...
        case 'ztdest':
          {
            const zt = _cleanHex(args[0][0])
            if (zt.length !== 10) return [args[0][1], args[0][2], 'Invalid ZeroTier address.']
            rules.push({
        ...
        case 'macdest':
          {
            const mac = _cleanMac(args[0][0])
            if (mac.length !== 17) return [args[0][1], args[0][2], 'Invalid MAC address.']
            rules.push({
              type: KEYWORD_TO_API_MAP[match],
              not: not,
              or: or,
        ...
        case 'ipdest':
          {
            ...
            const ipOnly = ip.substr(0, slashIdx)
        ...
        case 'icmp':
          {
            const icmpType = _parseNum(args[0][0])
            if (icmpType < 0 || icmpType > 0xff)
              return [args[0][1], args[0][2], 'Missing or invalid ICMP type.']
            const icmpCode = _parseNum(args[1][0]) // -1 okay, indicates don't match code
            if (icmpCode > 0xff)
        ...
        case 'chr':
          {
            const chrb = args[0][0].split(/[,]+/)
            let maskhi = 0
            let masklo = 0
            for (let i = 0; i < chrb.length; ++i) {
              if (chrb[i].length > 0) {
                const tmp = CHARACTERISTIC_BITS[chrb[i]]
                const bit = typeof tmp === 'number' ? tmp : _parseNum(chrb[i])
                if (bit < 0 || bit > 63)
                  return [
                    args[0][1],
                    args[0][2],
...
function _renderActions(rtree, rules, macros, caps, tags, params) {
  for (let k = 0; k < rtree.length; ++k) {
    const action = typeof rtree[k][0] === 'string' ? rtree[k][0].toLowerCase() : ''
    if (action.length === 0) {
      continue
      ...
      const macroParamArray = []
      const parenIdx = macroName.indexOf('(')
      if (parenIdx > 0) {
        const pns = macroName.substr(parenIdx + 1).split(/[,)]+/)
      ...
      const macroParams = {}
      for (const param in macro.params) {
        const pidx = macro.params[param]
      ...
      const err = _renderActions(macro.rules, rules, macros, caps, tags, macroParams)
      if (err !== null) return err
    } else if (action === 'drop' || action === 'accept' || action === 'break') {
      // actions without arguments
      if (k + 1 < rtree.length && Array.isArray(rtree[k + 1][0])) {
        const mtree = rtree[k + 1]
        ++k
        const err = _renderMatches(mtree, rules, macros, caps, tags, params)
        if (err !== null) return err
      }
      rules.push({
      ...
      if (k + 1 < rtree.length && Array.isArray(rtree[k + 1][0]) && rtree[k + 1][0].length >= 2) {
        const mtree = rtree[k + 1]
        ++k
        const maxLength = _parseNum(mtree[0][0])
        ...
        const err = _renderMatches(mtree.slice(2), rules, macros, caps, tags, params)
        if (err !== null) return err
        rules.push({
          type: KEYWORD_TO_API_MAP[action],
          address: target,
      ...
      if (k + 1 < rtree.length && Array.isArray(rtree[k + 1][0]) && rtree[k + 1][0].length >= 1) {
        ...
        const err = _renderMatches(mtree.slice(1), rules, macros, caps, tags, params)
        if (err !== null) return err
        rules.push({
          type: KEYWORD_TO_API_MAP[action],
          address: target,
...
function compile(src, rules, caps, tags) {
  try {
    if (typeof src !== 'string') return [0, 0, '"src" parameter must be a string.']
    ...
    for (let i = 0; i < parsed.length; ++i) {
      const keyword = typeof parsed[i][0] === 'string' ? parsed[i][0].toLowerCase() : null
      if (keyword === 'macro') {
        // Define macros
        ...
        if (!_isValidName(macroName)) return [macro[0][1], macro[0][2], 'Invalid macro name.']
        ...
        const tag = parsed[++i]
        const tagName = tag[0][0].toLowerCase()

        if (!_isValidName(tagName)) return [tag[0][1], tag[0][2], 'Invalid tag name.']
        if (tagName in RESERVED_WORDS) return [tag[0][1], tag[0][2], 'Tag name is a reserved word.']

        if (tagName in tags) return [tag[0][1], tag[0][2], 'Multiple definition of tag name.']
        ...
        for (let k = 1; k < tag.length; ++k) {
          const tkeyword = tag[k][0].toLowerCase()
          if (tkeyword === 'id') {
            if (id >= 0) return [tag[k][1], tag[k][2], 'Duplicate tag id definition.']
            if (k + 1 >= tag.length) return [tag[k][1], tag[k][2], 'Missing numeric value for ID.']
            id = _parseNum(tag[++k][0])
            if (id < 0 || id > 0xffffffff)
              return [tag[k][1], tag[k][2], 'Invalid or out of range tag ID.']
          } else if (tkeyword === 'default') {
            if (dfl !== null) return [tag[k][1], tag[k][2], 'Duplicate tag default directive.']
            if (k + 1 >= tag.length) return [tag[k][1], tag[k][2], 'Missing value for default.']
            dfl = tag[++k][0]
          } else if (tkeyword === 'flag') {
            if (k + 2 >= tag.length)
              return [tag[k][1], tag[k][2], 'Missing tag flag name or bit index.']
            ++k
            const bits = tag[k][0].split(/[,]+/)
            let mask = 0
            ...
            const flagName = tag[++k][0].toLowerCase()
        ...
        if (id < 0) return [tag[0][1], tag[0][2], 'Tag definition is missing a numeric ID.']
        ...
        const cap = parsed[++i]
        const capName = cap[0][0].toLowerCase()

        if (!_isValidName(capName)) return [cap[0][1], cap[0][2], 'Invalid capability name.']
        ...
        if (id < 0) return [cap[0][1], cap[0][2], 'Capability definition is missing a numeric ID.']
    ...
    for (const capName in caps) {
      const r = []
      const err = _renderActions(caps[capName].rules, r, macros, caps, tags, {})
      if (err !== null) return err
      caps[capName].rules = r
    }

    const err = _renderActions(baseRuleTree, rules, macros, caps, tags, {})
    if (err !== null) return err
