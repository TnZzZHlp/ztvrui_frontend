...
<template>
  ...
  >
    <div class="bg-white rounded min-w-lg min-h-50 p-5 flex flex-col justify-between">
      <span>
        <b>{{ message }}</b>
      </span>
      <div class="flex justify-end mt-4">
        <button
          class="mx-2 px-4 py-2 shadow-sm/20 rounded-md hover:bg-gray-200 transition-all active:bg-gray-300"
          @click="closePopupPanel"
        >
          <b>{{ t('cancel') }}</b>
        </button>
        <button
          class="mx-2 px-4 py-2 shadow-sm/20 rounded-md transition-all"
          :class="{
